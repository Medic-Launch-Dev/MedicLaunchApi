...
public static class IdentityApiEndpointRouteBuilderExtensions
{
		...
		where TUser : class, new()
	{
		...
		routeGroup.MapGet("/confirmEmail", async Task<Results<RedirectHttpResult, UnauthorizedHttpResult>>
			([FromQuery] string userId, [FromQuery] string code, [FromQuery] string? changedEmail, [FromServices] IServiceProvider sp, HttpContext httpContext) =>
		{
			var userManager = sp.GetRequiredService<UserManager<TUser>>();
			...
			{
				// Optionally log the error, but do not block confirmation
				// var logger = sp.GetRequiredService<ILogger<IdentityApiEndpointRouteBuilderExtensions>>();
				// logger.LogError(ex, "Error creating Stripe customer or MixPanel user after email confirmation.");
			}
